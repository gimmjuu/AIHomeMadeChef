...
class DBConnector:
    ...
    def add_preference(self, user_id: str, food_id: str):
        """찜하기 추가"""
        self.start_conn()
        sql = f"insert into \"TB_PREFER\"(\"USER_ID\", \"FOOD_ID\") values ('{user_id}', '{food_id}')"
    ...
    def remove_preference(self, user_id: str, food_id: str):
        """찜하기 취소"""
        self.start_conn()
        sql = f"delete from \"TB_PREFER\" where \"USER_ID\" = '{user_id}' and \"FOOD_ID\" = '{food_id}'"
    ...
    # === TB_RECIPE
    def insert_recipe_info(self):
        self.start_conn()
        sql1 = "select * from \"TB_RECIPE_1\""

        try:
            with self.DB.cursor() as cur:
                cur.execute(sql1)
                recipes = cur.fetchall()
                for recipe_id, food_nm in recipes:
                    sql2 = f"update \"TB_RECIPE_1\" set \"FOOD_ID\"='{food_id}' where \"RECIPE_NAME\" = '{food_nm}'"
                    cur.execute(sql2)
    ...
    def find_recipe_1_eq_food_nm(self):
        """음식 이름과 같은 레시피에 음식 아이디 추가->테스트 데이터"""
        self.start_conn()
    ...
    def find_recipe_by_food_id(self, t_id: str):
        """레시피 정보 조회"""
        self.start_conn()
        sql = f"select * from \"TB_RECIPE_1\" natural join \"TB_FOOD\" " \
              f"where \"FOOD_ID\" = '{t_id}'"
        ...
        result = Recipe(recipe_id=data[0], recipe_name=data[2], recipe_stuff=data[3], recipe_step=data[4])
        return result
...
if __name__ == '__main__':
    ...
    # db.insert_userinfo_to_table('lss', '111', '이승신')
    # db.update_user_taste_info('lss', '김치우동|계란찜')
